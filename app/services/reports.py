...
def dashboard_metrics(company_id, period="month"):
    ...
    try:
        ...
        # historical `except: pass` here silently emptied the panel on
        # ANY downstream failure (schema drift, join miss, etc.). That
        # was the "empty state that isn't actually empty" bug the
        # ticket points at. Log it now — never silent.
